...
虚函数会占类内存的原因是虚函数表指针占的内存，并参与内存对齐
...
使变量具有静态持续时间，在程序启动时分配，在程序结束时释放，只会被初始化一次，是线程安全的初始化

修饰全局变量会使得其他文件无法通过extern声明访问静态全局变量

使类成员变量被类所有实例共享同一个副本

使类成员函数可以通过类名调用，没有this指针，只能访问静态成员函数和静态成员变量

static成员不占类大小

##### 字符串

sizeof编译时确定大小，是计算分配的内存大小，strlen是计算'\0'前面有多少元素
...
将 数据（成员变量） 和 操作数据的方法（成员函数） 捆绑成一个类（Class），并控制外部对内部数据的访问权限（通过 `public`/`private`/`protected`）

#### 继承

允许一个类（派生类）基于另一个类（基类）扩展功能，实现 **代码复用** 和 **层次化设计**
...
将成员函数的函数指针作为结构体的数据成员，可以通过结构体调用对应的方法
...
将函数指针作为基类结构体的数据成员，创建派生类的时候将不同的函数指针赋值给同名函数指针
...
基本没区别，唯一的区别是结构体默认的成员访问权限是public，而类是private
...
同一个作用域中允许存在同名但参数列表不同的函数（类型、顺序、数量不同）

##### 隐藏

继承中派生类重写同名的非虚函数，可以使用基类作用域解析符调用被隐藏的函数
...
编译时代码替换，比宏定义更安全，有函数的语法检查、作用域、调试
...
const和引用成员无法在构造函数中初始化

初始化基类

#### 构造顺序

先构造基类，然后按照声明的顺序（即使在初始化列表中顺序不同）构造成员变量，最后执行构造函数
...
派生类继承基类的成员变量和成员函数，通过虚函数表管理虚函数

#### 虚函数实现原理

动态多态的实现依赖于虚函数，通过基类指针指向不同的派生类对象，可以正确调用对应派生类实现的虚函数

当类有虚函数的时候，编译器会为这个类生成一个虚函数表，这是一个指针数组，指向类的虚函数实现地址

如果子类重写虚函数，那么子类的虚函数表就会更新对应的虚函数指针

当生成派生类对象的时候会插入一个虚表指针指向派生类的虚函数表

通过基类指针调用虚函数的时候就可以根据这个虚表指针找到对应派生类的虚函数表，再根据虚函数的声明顺序找到对应的虚函数进行调用
...
可以将通用的操作封装在基类中，派生类可以重写自己的逻辑，程序运行时可以根据对象类型调用对应的函数
...
指向share指针管理的对象，只引用不计数，不能像前面两个一样调用reset释放资源
...
每个资源会对应一个控制块，在make_share指针的时候会给资源分配一个控制块，即所有指向同一对象的shared_ptr和weak_ptr共享同一个控制块
...
nullptr：NULL是宏定义为0或者（void*） 0，可以隐式转换成整数，在函数重载中可能被解析成整数类型，nullptr是std::nullptr_t类型，只能转换成指针
